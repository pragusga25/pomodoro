...
export function Timer({ timeLeft, isRunning, mode, totalTime, onToggle, onReset }: TimerProps) {
  ...
  return (
    <Card className="p-12 text-center shadow-lg border-2 bg-gradient-to-b from-background to-muted/20">
      <div className="space-y-8">
        <div>
          <h2 className="text-2xl font-semibold mb-2">{mode === "work" ? "Focus Time" : "Break Time"}</h2>
          <div className="text-8xl font-bold mb-6 text-primary tracking-tight">
            {formatTime(timeLeft)}
          </div>
        </div>
        <Progress
